...
impl PyPdu {
    ...
    pub fn serialize_header<'a>(
        &self,
        py: Python<'a>,
        buf: Option<&'a PyByteArray>,
        ...
        match buf {
            Some(buf) => {
                ...
                unsafe {
                    self.0
                        .unsafe_read(|pdu| pdu.serialize_header(&mut buf.as_bytes_mut()))?;
                }
                Ok(buf)
            }
            None => Ok(PyByteArray::new_with(py, len, |mut buf| {
                unsafe {
                    self.0.unsafe_read(|pdu| pdu.serialize_header(&mut buf))?;
                }
                Ok(())
            })?),
        }
    ...
    pub fn serialize_trailer<'a>(
        ...
        match buf {
            Some(buf) => {
                ...
                unsafe {
                    self.0
                        .unsafe_read(|pdu| pdu.serialize_trailer(&mut buf.as_bytes_mut()))?;
                }
                Ok(buf)
            }
            None => Ok(PyByteArray::new_with(py, len, |mut buf| {
                unsafe {
                    self.0.unsafe_read(|pdu| pdu.serialize_trailer(&mut buf))?;
                }
                Ok(())
            })?),
        }
    ...
    pub fn serialize<'a>(
        ...
        match buf {
            Some(buf) => {
                ...
                unsafe {
                    self.0
                        .unsafe_read(|pdu| pdu.serialize(&mut buf.as_bytes_mut()))?;
                }
                Ok(buf)
            }
            None => Ok(PyByteArray::new_with(py, len, |mut buf| {
                unsafe {
                    self.0.unsafe_read(|pdu| pdu.serialize(&mut buf))?;
                }
                Ok(())
            })?),
        }
